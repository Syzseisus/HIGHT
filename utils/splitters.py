...
null_value = 0  # float that specifies null value in data.y to filter if task_idx is provided
frac_train = 0.8
frac_valid = 0.1
frac_test = 0.1
...
def scaffold_split(dataset, smiles_list, task_idx=None, return_smiles=False):
    """
    ...
    all_scaffold_sets = [
        scaffold_set
        for (scaffold, scaffold_set) in sorted(all_scaffolds.items(), key=lambda x: (len(x[1]), x[1][0]), reverse=True)
    ]
...
def random_scaffold_split(dataset, smiles_list, seed=0, task_idx=None):
    """
...
def random_split(dataset, seed=0, task_idx=None, smiles_list=None):
    """
