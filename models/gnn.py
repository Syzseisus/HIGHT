...
# atom + motif/graph + mask
num_atom_type = 118 + 2 + 1
num_chirality_tag = 4
...
class GINConv(MessagePassing):
    ...
    def __init__(self, emb_dim, out_dim, aggr="add", **kwargs):
        ...
        self.mlp = nn.Sequential(nn.Linear(emb_dim, 2 * emb_dim), nn.ReLU(), nn.Linear(2 * emb_dim, out_dim))
...
class GNNDecoder(nn.Module):
    def __init__(self, hidden_dim, out_dim, JK="last", gnn_type="gin"):
        ...
        if gnn_type == "gin":
            ...
            raise NotImplementedError(f"{gnn_type} is not implemented.")
