...
from models import DiscreteGNN

"""
vqvae_args.py와 거의 비슷
1. `TrainingArguments.num_train_epochs`: 60 -> 100
2. `TrainingArguments.lr_scheduler_type`: "lambda" -> ""
3. `ModelArguments.vqvae_ckpt_path`: NEW
4. `ModelArguments.mu`: `beta`는 안쓰니까 교체
4. `ModelArguments.edge`: DELETE
5. `DataArguments.mask_rate1`: NEW
6. `DataArguments.mask_rate2`: NEW
7. `DataArguments.mask_edge`: NEW
"""
...
class ModelArguments:
    ...
    vqvae_ckpt_path: str = field(default=None, metadata={"help": "vqvae.py를 통해 얻은 ckpt 경로. 여기서 `encoder` 갖다 쓸거임"})

    def __post__init__(self):
        if not self.vqvae_ckpt_path:
            raise ValueError("`vqvae_ckpt_path`는 반드시 있어야 함")
        else:
            try:
                gnn = DiscreteGNN(5, 300, "last")
                gnn.from_pretrained(self.vqvae_ckpt_path)
            except FileNotFoundError:
                raise FileNotFoundError(f"{self.vqvae_ckpt_path}라는 파일 없어")
...
class DataArguments:
    ...
    mask_rate1: float = field(default=0.15, metadata={"help": "augmentation을 위한 masking hyperparameter - 1"})
    mask_rate2: float = field(default=0.30, metadata={"help": "augmentation을 위한 masking hyperparameter - 2"})
    mask_edge: bool = field(default=True, metadata={"help": "whether to mask edges or not together with atoms"})
